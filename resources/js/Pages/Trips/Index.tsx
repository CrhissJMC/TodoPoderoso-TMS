...
export default function TripsIndex({ trips, counts, routes, vehicles, drivers, filters, statuses }: Props) {
    ...
    const hasAdmin = permissions.includes('viajes.admin');
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {trips.data.length === 0 ? (
                                ...
                                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    {/* Estado */}
                                    <td className="px-4 py-3">
                                        <button onClick={() => hasAdmin && setStatusT(t)} disabled={!hasAdmin} className={`transition-opacity ${hasAdmin ? 'hover:opacity-75 cursor-pointer' : 'cursor-default'}`}>
